...
> 分类：LEGACY_PARTIAL_REFERENCE
>
> 本文件保留部分仍可追溯的数据模型设计，其中 20 类实体是旧版本历史快照。
>
> 当前产品目标已经由 V1.1 修订为 22 类实体。
>
> 本文件不能单独用于生成当前 migration、ORM 或 API schema。
>
> 当前实现必须以 V1.0 主基线与 V1.1 附属补丁联合基线为准。
>
> 在正式数据模型工作包完成前，本文件状态为 PARTIAL，不是 OBSOLETE，也不是当前权威入口。

本文旧版正文定义过 20 个核心实体的历史数据模型快照，并保留 MySQL、Redis Streams、DuckDB、原始文件卷和审计日志的部分参考设计。该 20 类实体不再代表当前目标；当前目标为 V1.1 修订后的 22 类实体。本文不构成实现完成声明，也不能单独作为当前权威数据模型。
