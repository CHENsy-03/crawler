...
历史文档曾记录：Python 1115 passed、7 skipped；
Go 全包 PASS；go vet PASS；go mod verify PASS。
上述文字引入于本地 Git 提交 a55ee6edb1304b307002445b1a83676b32739a1b。

该提交用于追溯文档记录，不等于测试执行日期或被测实现提交。
本次整理未确认这些结果所对应的实际执行时间、命令、环境及被测版本，
因此不将其作为当前测试通过的证据。

当前任务及其验证记录入口见 docs/TASK.md 的唯一现行入口。
验证结论应依据对应内容身份和适用范围的实际证据，
不能仅凭 TASK 状态判定测试通过。
本文后续测试目标与发布门禁保持有效。
...
| 测试能力 | 当前状态 | 目标 |
|---|---|---|
| Python/Go 组件测试 | 历史文档曾记录 PASS；当前结果须查阅对应任务的验证证据 | 保持并扩展 |
...
验证范围按当前任务阶段、变更风险和验收目标确定。
本文件的完整测试清单不作为每次任务的默认执行命令；
执行范围同时遵守 AGENTS.md §29、§33.1 及当前任务授权。

| 阶段 | 验证范围与边界 |
|---|---|
| 只读审查 | 不默认运行代码测试；明确要求的验证仍须执行，并遵守该任务允许的输出范围 |
| 文档修改 | 不默认重跑业务测试；完成与修改内容有关的文档、引用和差异验证，明确要求的其他验证仍须执行 |
| 代码修改 | 执行与变更风险和验收要求匹配的定向验证 |
| 隔离集成验证 | 在明确授权的环境和资源范围内执行规定验证，并记录资源与清理情况 |
| 发布级完整验收 | 在发布或明确要求完整验收的阶段执行全部适用门禁 |

定向验证不能代替明确要求的发布级门禁。
发布级门禁清单也不能被理解为每轮任务的默认命令。
本节不删除或降低已有安全、性能、恢复、E2E、canary、
数据隔离或发布阻塞要求。
...
复用既有证据时，应注明证据来源、对应内容身份、
覆盖范围、适用环境和复用理由。
相关内容、依赖、配置或环境变化可能使既有证据失效，
应先判断变化是否影响该证据的适用性。
...
不得把未执行写成通过，
不得仅凭 git diff、代码审阅或测试源码存在就声称测试通过。
