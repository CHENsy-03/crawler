...
$ComposePath = Join-Path $RepoRoot 'tests\integration\task019b8\compose.yml'
...
function New-UniqueProjectName {
    while ($true) {
        ...
        $candidate = 'crawler019b8-' + $hex
        if ($candidate -notmatch '^crawler019b8-[a-f0-9]{12}$') { continue }
...
function Get-ComposePort {
    ...
    if ($LASTEXITCODE -ne 0 -or -not $line) { throw "Unable to resolve port for $Service." }
    if ($line -notmatch '^127\.0\.0\.1:(\d+)$') { throw "Unexpected port mapping for $Service : $line" }
    return [int]$Matches[1]
}

function Invoke-GoTest {
    param([string]$Run, [switch]$Race)
    Push-Location $GoSpiderDir
    try {
        if ($Race) {
            & go test -race -mod=readonly -count=1 -run $Run './internal/worker'
        } else {
            & go test -mod=readonly -count=1 -run $Run './internal/worker'
        }
        if ($LASTEXITCODE -ne 0) { throw "Go test failed: $Run" }
    } finally {
        Pop-Location
    }
}
...
function Assert-NoResources {
    ...
    if ($containers.Count -ne 0 -or $networks.Count -ne 0 -or $volumes.Count -ne 0) {
        throw "Residual B8 resources: containers=$($containers.Count) networks=$($networks.Count) volumes=$($volumes.Count)"
    }
}

function Remove-TempEnv {
    foreach ($name in @('TASK019B8_E2E','TASK019B8_MYSQL_DSN','TASK019B8_REDIS_ADDR','TASK019B8_REPO_ROOT','TASK019B8_PYTHON_EXE','TASK019B8_MYSQL_ROOT_PASSWORD','TASK019B8_MYSQL_USER_PASSWORD')) {
...
try {
    ...
    $env:TASK019B8_MYSQL_ROOT_PASSWORD = ($rootBytes | ForEach-Object { $_.ToString('x2') }) -join ''
    $env:TASK019B8_MYSQL_USER_PASSWORD = ($userBytes | ForEach-Object { $_.ToString('x2') }) -join ''
    $env:TASK019B8_REPO_ROOT = $RepoRoot
    $env:TASK019B8_PYTHON_EXE = (Get-Command python).Source
    ...
    $env:TASK019B8_MYSQL_DSN = "crawler_b8:$($env:TASK019B8_MYSQL_USER_PASSWORD)@tcp(127.0.0.1:$mysqlPort)/crawler_b8?charset=utf8mb4&parseTime=True"
    $env:TASK019B8_REDIS_ADDR = "127.0.0.1:$redisPort"

    Invoke-GoTest '^TestTask019B8LegacyBootstrap$'

    $migrationCommand = 'MYSQL_PWD="$MYSQL_PASSWORD" mysql -h127.0.0.1 -ucrawler_b8 crawler_b8 < ' + $MigrationContainerPath
    & docker compose --project-name $ProjectName --file $ComposePath exec -T mysql sh -c $migrationCommand
    if ($LASTEXITCODE -ne 0) { throw 'B5 migration failed.' }

    Invoke-GoTest '^TestTask019B8SchemaAfterMigration$'
    Invoke-GoTest '^TestTask019B8FullChain$' -Race
}
catch {
    Write-Error $_
    if ($ProjectName) {
    ...
    if ($ProjectName) {
        ...
        while ($downAttempts -lt 2) {
            try {
                ...
                $downAttempts++
                if ($downAttempts -ge 2) { Write-Error $_; throw }
